...
def test_registro_cliente_usuario_duplicado():
    User.objects.create_user(
        username="existente", email="e@test.com", password="pass123"
    )
    response = client.post(
...
def test_login_exitoso():
    ...
    response = client.post("/login?username=loginuser&password=pass123")
    assert response.status_code == 200
    data = response.json()
...
def test_login_credenciales_incorrectas():
    response = client.post("/login?username=noexiste&password=wrongpass")
    assert response.status_code == 200
    assert "error" in response.json()
...
def test_usuario_factory_crea_cliente():
    user = UsuarioFactory.crear_usuario(
        tipo="cliente", nombre="Juan", correo="juan@test.com"
    )
...
def test_usuario_factory_crea_repartidor():
    user = UsuarioFactory.crear_usuario(
        tipo="repartidor", nombre="Pedro", correo="pedro@test.com"
    )
...
def test_usuario_factory_crea_admin():
    user = UsuarioFactory.crear_usuario(
        tipo="admin", nombre="Jefe", correo="jefe@test.com"
    )
...
def test_usuario_factory_invalido():
    with pytest.raises(ValueError, match="Tipo de usuario no válido"):
        UsuarioFactory.crear_usuario(
            tipo="vendedor", nombre="Error", correo="error@test.com"
        )
